function E_K_BR_RU_BU_l_w_h_theta = E_K_BR_RU_BU_l_w_h_theta_function(l,w,h,theta,x_U_1,x_U_2,x_n_1,x_n_2,H_max,Theta_max,H_B,H_U,h_R,x_B_1,x_B_2,lambda)
%This function calculates the mean number of blockages in the BR, RU
%and/or in the BU link.
[P,Q]=size(h);
E_K_BR_RU_BU_l_w_h_theta=zeros(P,Q);
...
x_B_BU_1_1 = zeros(P,Q)+real(x_B_1+1i*x_B_2+((x_U_1+1i*x_U_2-(x_B_1+1i*x_B_2)).*(H_B-h)./(H_B-H_U))).*(h>=H_U & h<H_B)+x_B_1.*(h>=H_B)+a.*cos(gamma_1).*(h>=H_U);
x_B_BU_2_1 = zeros(P,Q)+imag(x_B_1+1i*x_B_2+((x_U_1+1i*x_U_2-(x_B_1+1i*x_B_2)).*(H_B-h)./(H_B-H_U))).*(h>=H_U & h<H_B)+x_B_2.*(h>=H_B)+a.*sin(gamma_1).*(h>=H_U);

x_B_BU_1_2 = zeros(P,Q)+real(x_B_1+1i*x_B_2+((x_U_1+1i*x_U_2-(x_B_1+1i*x_B_2)).*(H_B-h)./(H_B-H_U))).*(h>=H_U & h<H_B)+x_B_1.*(h>=H_B)+a.*cos(gamma_2).*(h>=H_U);
x_B_BU_2_2 = zeros(P,Q)+imag(x_B_1+1i*x_B_2+((x_U_1+1i*x_U_2-(x_B_1+1i*x_B_2)).*(H_B-h)./(H_B-H_U))).*(h>=H_U & h<H_B)+x_B_2.*(h>=H_B)+a.*sin(gamma_2).*(h>=H_U);

x_B_BU_1_3 = zeros(P,Q)+real(x_B_1+1i*x_B_2+((x_U_1+1i*x_U_2-(x_B_1+1i*x_B_2)).*(H_B-h)./(H_B-H_U))).*(h>=H_U & h<H_B)+x_B_1.*(h>=H_B)+a.*cos(gamma_3).*(h>=H_U);
x_B_BU_2_3 = zeros(P,Q)+imag(x_B_1+1i*x_B_2+((x_U_1+1i*x_U_2-(x_B_1+1i*x_B_2)).*(H_B-h)./(H_B-H_U))).*(h>=H_U & h<H_B)+x_B_2.*(h>=H_B)+a.*sin(gamma_3).*(h>=H_U);

x_B_BU_1_4 = zeros(P,Q)+real(x_B_1+1i*x_B_2+((x_U_1+1i*x_U_2-(x_B_1+1i*x_B_2)).*(H_B-h)./(H_B-H_U))).*(h>=H_U & h<H_B)+x_B_1.*(h>=H_B)+a.*cos(gamma_4).*(h>=H_U);
x_B_BU_2_4 = zeros(P,Q)+imag(x_B_1+1i*x_B_2+((x_U_1+1i*x_U_2-(x_B_1+1i*x_B_2)).*(H_B-h)./(H_B-H_U))).*(h>=H_U & h<H_B)+x_B_2.*(h>=H_B)+a.*sin(gamma_4).*(h>=H_U);
...
for p=1:P
    for q=1:Q
        %Then, we obtain the S_BR_l_w_h_theta polygon:
        ...
        S_BU_l_w_h_theta_1 = polyshape([x_B_BU_1_1(p,q) x_B_BU_1_2(p,q) x_B_BU_1_3(p,q) x_B_BU_1_4(p,q)],[x_B_BU_2_1(p,q) x_B_BU_2_2(p,q) x_B_BU_2_3(p,q) x_B_BU_2_4(p,q)]);
        S_BU_l_w_h_theta_2 = polyshape([x_U_1_1(p,q) x_U_1_2(p,q) x_U_1_3(p,q) x_U_1_4(p,q)],[x_U_2_1(p,q) x_U_2_2(p,q) x_U_2_3(p,q) x_U_2_4(p,q)]);
        S_BU_l_w_h_theta_3 = polyshape([x_B_BU_1_1(p,q) x_B_BU_1_3(p,q) x_U_1_3(p,q) x_U_1_1(p,q)],[x_B_BU_2_1(p,q) x_B_BU_2_3(p,q) x_U_2_3(p,q) x_U_2_1(p,q)]);
        S_BU_l_w_h_theta_4 = polyshape([x_B_BU_1_2(p,q) x_B_BU_1_4(p,q) x_U_1_4(p,q) x_U_1_2(p,q)],[x_B_BU_2_2(p,q) x_B_BU_2_4(p,q) x_U_2_4(p,q) x_U_2_2(p,q)]);
        
        S_BU_l_w_h_theta = union(union(S_BU_l_w_h_theta_1,S_BU_l_w_h_theta_2),union(S_BU_l_w_h_theta_3,S_BU_l_w_h_theta_4));
        
        E_K_BR_RU_BU_l_w_h_theta(p,q) = lambda*area(union(union(S_BR_l_w_h_theta,S_RU_l_w_h_theta),S_BU_l_w_h_theta))*1/H_max*1/Theta_max;
    end
end
end
