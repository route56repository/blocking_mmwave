...
lambda = 3.1e-4:5e-5:5e-4;

V = length(lambda);

N_runs = 200;

P_allKO_simulation = zeros(1,V);

for v = 1:V  
    for n_run = 1:N_runs
        %We calculate both the number and the characteristics of the blocking
        %elements.
        B = poissrnd(lambda(v)*pi*(1.2*R)^2); %It calculates the number of buildings of
        ...
        P_allKO_simulation(v) = P_allKO_simulation(v)+sum(BS_UE_blocked==1)/num_pos;
        save('n_run.mat','n_run');
        n_run
        save('P_allKO_simulation.mat','P_allKO_simulation');
    end
    P_allKO_simulation(v) = P_allKO_simulation(v)/N_runs;
    save('v.mat','v');
    v
    save('P_allKO_simulation.mat','P_allKO_simulation');
end
